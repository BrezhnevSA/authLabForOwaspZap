...
var API_URL = "http://script-token-timeout:3000";

// Нужны правки, реальный URL логина
var TOKEN_URL = "http://script-token-timeout:3000/api/login";
...
// Возможно нужны правки, если токен протухает быстрее\медленее
var TOKEN_RENEW_AFTER_SECONDS = 5;
...
function tokenIsValid() {
    ...
    var refreshAt = Number(ScriptVars.getScriptVar(SCRIPT_CONTEXT, EXPIRY_IN_MS_KEY));
    return token && isFinite(refreshAt) && Date.now() < refreshAt;
}
...
function refreshToken(helper) {
    ...
    try {
        ...
        ScriptVars.setScriptVar(SCRIPT_CONTEXT, EXPIRY_IN_MS_KEY, String(Date.now() + TOKEN_RENEW_AFTER_SECONDS * 1000));
        log("Token refreshed");
        return true;
    } catch (error) {
