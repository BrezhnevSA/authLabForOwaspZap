...
var API_URL = "http://otp-challenge:3000";
...
var OTP="123456";
function txFromLocation(location) {
    if (!location) return null;
    var m=String(location).match(/[?&]tx=([^&]+)/);
    return m ? decodeURIComponent(m[1]) : null;
}
function login(helper) {
    var body="username=" + formEncode(USERNAME) + "&password=" + formEncode(PASSWORD);
    var first=newMessage(HttpRequestHeader.POST, API_URL + "/login", "application/x-www-form-urlencoded", body, null);
    helper.getHttpSender().sendAndReceive(first, false);
    if (!requireStatus(first,[302])) return null;
    var tx=txFromLocation(first.getResponseHeader().getHeader("Location"));
    if (!tx) { log("OTP transaction id not found"); return null; }
    var otpBody="tx=" + formEncode(tx) + "&otp=" + formEncode(OTP);
    var second=newMessage(HttpRequestHeader.POST, API_URL + "/otp", "application/x-www-form-urlencoded", otpBody, null);
    helper.getHttpSender().sendAndReceive(second, false);
    if (!requireStatus(second,[302])) return null;
    return cookieFromResponse(second);
}
