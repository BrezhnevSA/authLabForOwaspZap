...
var API_URL = "http://go-multistep:8080";
...
function txFromHtml(html) {
    var m=String(html).match(/name=["']tx["'][^>]*value=["']([^"']+)["']/i);
    if (!m) m=String(html).match(/value=["']([^"']+)["'][^>]*name=["']tx["']/i);
    return m ? m[1] : null;
}
function login(helper) {
    var first=newMessage(HttpRequestHeader.POST, API_URL + "/login/username", "application/x-www-form-urlencoded", "username=" + formEncode(USERNAME), null);
    helper.getHttpSender().sendAndReceive(first, false);
    if (!requireStatus(first,[200])) return null;
    var tx=txFromHtml(first.getResponseBody());
    if (!tx) { log("Transaction id not found"); return null; }
    var body="tx=" + formEncode(tx) + "&password=" + formEncode(PASSWORD);
    var second=newMessage(HttpRequestHeader.POST, API_URL + "/login/password", "application/x-www-form-urlencoded", body, null);
