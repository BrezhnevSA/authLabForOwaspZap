...
var API_URL = "http://spring-form:8080";
...
function csrfFromHtml(html) {
    ...
    var m=text.match(/name=["']_csrf["'][^>]*value=["']([^"']+)["']/i);
    if (!m) m=text.match(/value=["']([^"']+)["'][^>]*name=["']_csrf["']/i);
    return m ? m[1] : null;
}
function login(helper) {
    var page=newMessage(HttpRequestHeader.GET, API_URL + "/login", null, null, null);
    helper.getHttpSender().sendAndReceive(page, false);
    if (!requireStatus(page,[200])) return null;
    var cookie=cookieFromResponse(page);
    var csrf=csrfFromHtml(page.getResponseBody());
    if (!csrf) { log("CSRF token not found"); return null; }
    var body="_csrf=" + formEncode(csrf) + "&username=" + formEncode(USERNAME) + "&password=" + formEncode(PASSWORD);
    var submit=newMessage(HttpRequestHeader.POST, API_URL + "/login", "application/x-www-form-urlencoded", body, cookie);
    helper.getHttpSender().sendAndReceive(submit, false);
    if (!requireStatus(submit,[302])) return null;
    return cookieFromResponse(submit) || cookie;
}
